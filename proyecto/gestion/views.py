...
#ORM
#SELECT * FROM empleados JOIN departamento ON empleados.departamento_id=departamento.id'''
...
def asignar_empleado(request):
    try:
        ...
        #SELECT *FROM departamento WHERE nombre="Recursos Humanos";
    except ObjectDoesNotExist:
        return render(request, 'gestion/error.html', {'mensaje': 'El departamento de Recursos Humanos no existe'})
    
    empleado, creado=Empleado.objects.get_or_create(
        correo='ana@gmail.com', 
        defaults={'nombre': 'Ana Lopez', 'departamento': departamento}
        )
